...
export default function Pdf2ExcelScreen() {
  ...
  const convertFile = async () => {
    ...
    try {
      ...
      const response = await api.post('/api/pdf2excel', formData, {
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.header}>
            <Text style={styles.description}>Convert PDF documents to Excel spreadsheets.</Text>
        </View>

        {!file ? (
          <TouchableOpacity style={styles.uploadArea} onPress={pickFile}>
            <Ionicons name="cloud-upload-outline" size={48} color="#14B8A6" />
      ...
      <View style={styles.footer}>
        ...
        >
          {loading ? <ActivityIndicator color="white" /> : <Text style={styles.actionBtnText}>Convert to Excel</Text>}
        </TouchableOpacity>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  uploadArea: {
    borderWidth: 2, borderColor: '#14B8A6', borderStyle: 'dashed',
    borderRadius: 16, height: 200, alignItems: 'center', justifyContent: 'center', backgroundColor: '#F0FDFA'
  },
  uploadText: { marginTop: 12, color: '#0D9488', fontWeight: '600' },
  fileCard: { 
  ...
  actionButton: { backgroundColor: '#14B8A6', padding: 16, borderRadius: 12, alignItems: 'center' },
  disabledButton: { backgroundColor: '#99F6E4' },
